...
export const getObras = async (
  ...
  try {
    ...
    const [obras, total] = await Promise.all([
      ...
      prisma.obraEjecucion.count({ where })
    ])
    ...
    res.json({
      success: true,
      data: response,
      message: `${obras.length} obras de ejecución encontradas`
    })
  } catch (error) {
    next(error)
  }
...
export const getObraById = async (
  ...
  try {
    ...
    const obra = await prisma.obraEjecucion.findUnique({
    ...
    if (!obra) {
      return res.status(404).json({
        ...
        error: {
          code: 'NOT_FOUND',
          message: 'Obra de ejecución no encontrada'
        }
      })
    }
    ...
    res.json({
      success: true,
      data: obraResponse,
      message: 'Obra de ejecución encontrada'
    })
  } catch (error) {
    next(error)
  }
...
export const createObra = async (
  ...
  try {
    ...
    const obra = await prisma.obraEjecucion.create({
      data: {
        ...obraData,
    ...
    res.status(201).json({
      success: true,
      data: obraResponse,
      message: 'Obra de ejecución creada exitosamente'
    })
  } catch (error) {
    next(error)
  }
...
export const updateObra = async (
  ...
  try {
    ...
    const obraExistente = await prisma.obraEjecucion.findUnique({
      where: { id: parseInt(id) }
    })
    ...
    const obra = await prisma.$transaction(async (tx) => {
      ...
      const obraActualizada = await tx.obraEjecucion.update({
        where: { id: parseInt(id) },
        data: dataToUpdate
      })
      ...
      if (profesionales) {
        ...
        await tx.profesionalEjecucion.deleteMany({
          where: { obraId: parseInt(id) }
        })

        // Crear nuevos profesionales
        await tx.profesionalEjecucion.createMany({
      ...
      return tx.obraEjecucion.findUnique({
        where: { id: parseInt(id) },
        include: {
          empresaEjecutora: true,
          empresaSupervisora: true,
    ...
    res.json({
      success: true,
      data: obraResponse,
      message: 'Obra de ejecución actualizada exitosamente'
    })
  } catch (error) {
    next(error)
  }
...
export const deleteObra = async (
  ...
  try {
    ...
    await prisma.obraEjecucion.delete({
      where: { id: parseInt(id) }
    })

    res.json({
      success: true,
      message: 'Obra de ejecución eliminada exitosamente'
    })
  } catch (error) {
    next(error)
  }
...
export const getProfesionales = async (
  ...
  try {
    ...
    const profesionales = await prisma.profesionalEjecucion.findMany({
      where: { obraId: parseInt(id) },
      orderBy: { porcentajeParticipacion: 'desc' }
    })
...
export const addProfesional = async (
  ...
  try {
    ...
    const profesionalesExistentes = await prisma.profesionalEjecucion.findMany({
      where: { obraId: parseInt(id) }
    })
    ...
    const profesional = await prisma.profesionalEjecucion.create({
      data: {
        obraId: parseInt(id),
        ...profesionalData
      }
...
export const updateProfesional = async (
  ...
  try {
    ...
    const profesionalExistente = await prisma.profesionalEjecucion.findUnique({
      where: { id: parseInt(id) }
    })
    ...
    if (updateData.porcentajeParticipacion !== undefined) {
      const otrosProfesionales = await prisma.profesionalEjecucion.findMany({
        where: { 
    ...
    const profesional = await prisma.profesionalEjecucion.update({
      where: { id: parseInt(id) },
      data: updateData
    })
...
export const deleteProfesional = async (
  ...
  try {
    ...
    await prisma.profesionalEjecucion.delete({
      where: { id: parseInt(id) }
    })
