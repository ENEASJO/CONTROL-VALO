...
import { useSupervisionObra } from '../../hooks/useSupervision'
import LoadingSpinner from '../../components/Common/LoadingSpinner.tsx'
import ErrorMessage from '../../components/Common/ErrorMessage.tsx'

const SupervisionObraDetalle = () => {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const obraId = id ? parseInt(id) : 0

  const { data: obraData, isLoading, error, refetch } = useSupervisionObra(obraId)
  ...
  if (error || !obraData?.success) {
    return (
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
          ...
          <Box>
            <Typography variant="h4" fontWeight="bold">
              Detalle de Obra de Supervisión
            </Typography>
        ...
        <ErrorMessage
          message="Error al cargar los detalles de la obra de supervisión"
          onRetry={() => refetch()}
          fullWidth
        />
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              Obra de Supervisión - {obra.numeroContrato}
            </Typography>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Button
            ...
            startIcon={<EditIcon />}
            sx={{ bgcolor: '#f57c00' }}
            onClick={() => navigate(`/supervision/${obra.id}/editar`)}
          >
            Editar
          </Button>
        </Box>
          ...
          <Card>
            <CardContent>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, bgcolor: '#f57c0015', borderRadius: 2 }}>
                    <SupervisionIcon sx={{ color: '#f57c00' }} />
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        Empresa Supervisora
                      </Typography>
...
export default SupervisionObraDetalle